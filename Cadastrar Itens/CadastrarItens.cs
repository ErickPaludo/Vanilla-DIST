namespace Vanilla
{
    internal class CadastrarItens : CadastraCnpjBack
    {
        ...
        public CadastrarItens(int id_item, int id_fornec,string nome_f, string nome_item, decimal preco_custo, decimal lucro_porcent, decimal preco_final, int quantidade, string codbar, string status_conv, string descricao, string unmed) : base(nome_f,id_fornec)
        {
        ...
        public void CadastraItem(int id_fornecedor, string codigo_barras, string nome, string status, string desc, string und_m, int quant, decimal preco_custo, decimal margem_lucro, decimal preco_venda) //Grava no banco de dados
        {
            db.GravarItens (id_fornecedor, codigo_barras, nome, status, desc, und_m, quant, preco_custo, margem_lucro, preco_venda); //grava de fato no banco
        }
        public void EditaItens(int id_principal,int id_fornecedor,string cod,string nome, string status, string desc, string und_m, int quant, decimal preco_custo, decimal margem_lucro, decimal preco_venda) //Grava no banco de dados
        {
            db.EditaItens(id_principal, id_fornecedor, cod, nome, status, desc, und_m, quant, preco_custo, margem_lucro, preco_venda); 
        }
        public void ReturItens(int id)
        {
            db.ChamaView("view_itens");
        }
    }
}
